...
Default required: A class that extends Effect.Service must declare one of the built-in constructors (effect, sync, succeed, or scoped). This baseline implementation becomes part of MyService.Default, so any code that imports the service can run without providing extra layers. That is handy for app-level services where a sensible runtime implementation exists (logging, HTTP clients, real databases, and so on). If your service is inherently contextual (for example, a per-request database handle) or you are writing a library that should not assume an implementation, prefer Context.Tag: you publish only the tag and let callers supply the layer that makes sense in their environment.
