...
Please note that to define the target schema, we used Schema.typeSchema. This is because the decoding/encoding of the elements is already handled by the from schema: Schema.Array(itemSchema), avoiding double decoding.
...
Using Type Assertion: Adding a type cast can enforce the return type to be treated as type A:

decode: (a) => Number.clamp(a, { minimum, maximum }) as A

Using the Non-Strict Option: Setting strict: false in the transformation options allows the schema to bypass some of TypeScript’s type-checking rules, accommodating the type discrepancy:
