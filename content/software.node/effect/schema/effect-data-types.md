...
Config
The Schema.Config function allows you to decode and manage application configuration settings using structured schemas. It ensures consistency in configuration data and provides detailed feedback for decoding errors.
...
It returns a Config object that integrates with your application’s configuration system.
...
Fetch the value using the provided name (e.g. from an environment variable).
Decode the value using the given schema. If the value is invalid, decoding fails.
Format any errors using TreeFormatter.formatErrorSync, which helps produce readable and detailed error messages.
