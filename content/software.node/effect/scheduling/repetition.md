...
Initial Execution Included

The scheduled recurrences are in addition to the initial execution, so Effect.repeat(action, Schedule.once) executes action once initially, and if it succeeds, repeats it an additional time.
...
You can use Effect.retry if you need to set conditions based on error occurrences rather than success outcomes.
